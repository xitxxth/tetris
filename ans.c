...
int getRecY(char recField[HEIGHT][WIDTH], int y, int x, int blockID, int blockRotate)
{
	int rec_y;
	...
	{
		y++;
		if (CheckToMove(recField, blockID, blockRotate, y, x) == 0)
			break;
	}
	rec_y = y - 1;
...
int recommend(Tree *Root)
{
	...
	for (rotate = 0; rotate < 4; rotate++)
	{
		...
		for (x = -3; x < WIDTH; x++)
		// 블록을 놓을 수 있는 WIDTH 범위 구하기
		{
			// 블록을 놓을 수 있으면 node에 추가하기
			if (CheckToMove(Root->recField, nextBlock[Root->level], rotate, 0, x) == 1)
			{
				score = 0;
				child[i] = (Tree *)malloc(sizeof(Tree));
				memcpy(child[i]->recField, Root->recField, sizeof(field));
				child[i]->level = Root->level + 1;

				y = getRecY(child[i]->recField, 0, x, nextBlock[Root->level], rotate);
				score += AddBlockToField(child[i]->recField, nextBlock[Root->level], rotate, y, x);
				score += DeleteLine(child[i]->recField);

				if (child[i]->level < VISIBLE_BLOCKS)
				{
					score += recommend(child[i]);
				}
				if (max <= score)
				{
					Root->recBlockX = x;
...
void sort_scores(int arrVal[], int arrIdx[], int arrSize)
{
	// 배열과 배열의 index를 오름차순으로 정렬해주는 함수
	int temp, tempIdx;
...
int modified_recommend(Tree *Root)
{
	...
	int scores[CHILDREN_MAX] = {
		0,
	};
	int scores_idx[CHILDREN_MAX] = {
		0,
	};
	...
	{
		sort_scores(scores, scores_idx, CHILDREN_MAX);

		for (j = CHILDREN_MAX - 1; j >= CHILDREN_MAX - TOP_RANK; j--)
		{
			i = scores_idx[j];
			score = child[i]->currentScore;

			if (child[i]->level < VISIBLE_BLOCKS)
			{
				score += modified_recommend(child[i]);
			}
			if (max <= score)
			{
				Root->recBlockX = child[i]->x;
				Root->recBlockY = child[i]->y;
				Root->recBlockRotate = child[i]->r;
